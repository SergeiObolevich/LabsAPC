...
void Server(char* path) {
	...
	CreateProcess(
		...
		" Client",															// Параметры командной строки.
	...
	CommTimeOuts.ReadTotalTimeoutConstant = TIMEOUT;						// Константа, используемая, чтобы вычислить полный (максимальный) период времени простоя для операций чтения.
	...
	CommTimeOuts.WriteTotalTimeoutConstant = TIMEOUT;						// Константа, используемая, чтобы вычислить полный период времени простоя для операций записи.
